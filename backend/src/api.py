...
def after_request(response):
    ...
    response.headers.add('Access-Control-Allow-Methods',
                             'GET,PUT,PATCH,POST,OPTIONS,DELETE')
...
def patch_drinks(payload, id):
    ...
    # Get selected drink from database
    drink_by_id = Drink.query.filter(Drink.id==id).first()
    ...
    if drink_by_id is None:
        ...
        try:            
            ...
            # Update records based on values from form fields
            drink_by_id.title = title
            drink_by_id.recipe = json.dumps(recipe) 
            drink_by_id.update()
                
            # Get the updated drink

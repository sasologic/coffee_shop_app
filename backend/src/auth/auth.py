...
path = os.path.realpath(os.path.join(os.path.dirname(__file__),'..','database','.env'))
...
def verify_decode_jwt(token):
    ...
    verify_decode_jwt(token) method
        ...
        it verifie the token using Auth0 /.well-known/jwks.json
...
def check_permissions(permission, payload):
    ...
    @INPUTS
        ...
        it validates claims and check the requested permission
        returns True else it raises AuthError 400 or 403 depending
    '''
